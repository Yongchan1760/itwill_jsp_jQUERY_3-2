...
$(function(){
  ...
  //하위 모든 <dd>태그들을 배열형식으로 담아 선택 해 오는데..
  //그중에서  첫번쨰 <dd>태그(첫번째 배너)를 제외한 나머지 배너들을 최종적으로 선택해서 
  //첫번쨰 배너dd를 제외하고 숨김니다.
  ...
  //id속성값이 roll_banner_wrap인 <div>태그의 하위 모든 <dt>태그의  하위 모든 <a>태그를 선택해서
  //( [1] , [2], [3], [4] <img>를 포함하고 있는 <a>태그들을 배열에 담아서 선택해옴)
  //그중에서 하나를 클릭 했을때...
  $("#roll_banner_wrap dt a").on("click", function(){
	  
	  /*onBtn변수에 저장되어 있는 <a>태그의 하위 <img>태그를 선택해  
	  "src"속성의 값을 비활성화된 이미지 경로로 바꿉니다*/
	  
	  //노출 되어 있는 배너 이미지 영역을 숨깁니다.
	  $("#roll_banner_wrap dd:visible").hide();
	  
	  
	  //변수 onBtn에 저장되어 있는 <a>태그의 하위 버튼<img>태그를 선택해
	  //attr("속성명",바꿀경로값)메소드를 이용하여 비활성화된 이미지로 바꾸자 
	  $("img",onBtn).attr("src", 
			  			  $("img",onBtn).attr("src").replace("over.gif","out.gif"));
	  
	  //[1]~[4]번 버튼 이미지중.. 클릭한 <a>태그를 선택해  그 <a>태그의 index위치값을 구해 옵니다.
	  //예를 들어 ,  [1]번 버튼을 클릭했을때  변수 num에는 0이 저장되고,
	  //          [3]번 버튼을 클릭했을때 변수num에는 2가 저장되게 하자.
	  var num = $("#roll_banner_wrap dt a").index(this);
	  
	  //클릭한  버튼<a>에 인덱스 위치값과 일치하는 <dd>태그영역(배너이미지영역)만 화면에 나타나게 하기 (노출)
	  //풀이: 변수 num에 저장된 클릭한 <img>를 감싸고 있는 <a>의 인덱스 값을 이용해
	  //     해당하는 배너 <img>를 포함하고 있는 <dd>태그영역을 선택해  show()메소드를 이용하여 노출 시키자
 ...
 function autoPlay(){ //1~4버튼이 순서대로 강제 클릭 이벤트가 발생되게 하는 함수 
	 
	 //일정한 간격으로 실행문이 실행될떄마다 변수 btnNum의 값이 1씩 증가 됩니다.
	 btnNum++;
	 
	 /*
	 	Auto 동작 원리에 대해서 ...
	 	버튼은 총 4개로, 1번 부터 4번까지 차례대로 일정한 시간간격으로 강제 클릭이벤트가 반복되어 발생된다면
	 	자동으로 배너 이미가 바뀌게 될것이다.
	 	이때 버튼<img>를 감싸고 있는<a>의 인덱스 위치값은 0부터 3까지 입니다.
	 	그러므로 일정한 간격으로 인덱스 위치값이  0부터 1씩 증가되고
	 	값이 4이상이 되면 다시 0로 돌려 강제 이벤트를 발생 시킴
	  */
	  //만약  btnNum변수의 값이 4이상이 되면  btnNum변수의 값을 0으로 초기화시킴
	  if(btnNum >= 4) btnNum = 0;
	  
	  //-변수 btnNum에 저장된 값을 이용해 버튼 <img>를 감싸고 있는<a>를 차례로 선택해 온후
	  // trigger("이벤트명")함수를 적용하여  강제로 클릭 이벤트를 발생 시킵니다.
 	...
 	//최초 로딩시 3초 이후에 autoPlay함수를 호출하여 실행합니다
 ...
 */  
  $(".stopBtn").on("click",function(){
	 //정지하려면 오토1 호출문을 없애주자  
	  clearTimeout(auto1);
	  //자동으로 배너가 흘러가지않는다 강제 클릭이 안된다.
	  
	  //정지 버튼 이미지 버튼을 가져와서 버튼이 클릭됬을떄 활성화된 컬러 정지 이미지로 바꿔야한다.
	  $("img",this).attr("src","images/pop_btn_stop_on.gif");//
	  
	  
	  //그리고 재생버튼 이미지 태그의 src속성 값을 변경하여 비활성화된 흑백 이미지로 바꿉니다.
  ...
  $(".playBtn").click(function(){
	 //방문자가 재생 버튼을 클릭하게 되면 auto1변수에 저장된 setTimeout()함수는
	  //스택에 실행문이 여러번 쌓여 문제가 될수있다.
	  //이문제를 해결하기 위해서는 clear Timelout() 메소드를 사용해 이미 앞에 저장된 setTimeout함수를 삭제한후
	  //setTimeout()함수를 실행해야합니다
	  //이 결과, 여러번 중복되어 스택에 쌓이는 것을 방지 할수있게됨
	  clearTimeout(auto1);//auto변수에 할당된settimeout함수를 스택 메모리에서 제거 시킴
	  auto1=setTimeout(autoPlay(), 1000);//1초뒤에 autoplay함수를 호출하여 실행시키자.
	  
	  //재생 버튼의 img의 src속성값을 바꾸어 활성화된 컬러이미지로 바꾸자 
	  $("img",this).attr("src","images/pop_btn_play_on.gif");
	  //활성화된 정지 버튼의 img의 src속성의 값을 바꾸어 비활성화된 정지 버튼 이미지로 바꾸자 
 ...
 //-----------------------------------------------------------
  ...
  //id속성값이 tabmenu인 dl id=tabmenu 태그의 하위 dt태그의 하위 첫번쨰 a태그를 선택해서 
  // 요약 :ㅣ초기에 활성화된 첫번쨰 탭 a만 선택해서 변수 ontab에 할당 
  var onTab=$("#tabmenu dt a:first")//텝메뉴의ㅏ 디티테그안의 a영역ㅇ으ㅢ 첫번쨰값을 탭에 저장
  
  //탭영역을 나타내는 a태그들 (공지상항,질문과 답변, 저자 문의 )중에 .. 마우스를 올리거나 포커스를 생성시키거나 클릭했을떄....
  $("#tabmenu dt a").on("mouseover focus click",function(){
	  //먼저 현재 노출된 :visible되어있는 태그 영역(게시글 목록화면 ) 을 숨긴다.
	  //현재 보이는 게시물 목록을 숨김 
	  $("#tabmenu dd:visible").hide();
	  //ontab변수에 할당된 공지사항 a태그의 하위 img태그를 선택해 src속성값을 바꾸어 
	  //비활성화된 공지사항 이미지로 바꾼다 
	  
	  $("img",onTab).attr("src",$("img",onTab).attr("src").replace("over.gif","out.gif"))//img중 하위 ontab을 선택해서
	  
	  //마우스를 올리거나 클릭한 a태그( 공지사항or질문과 답변or저자문의 중 하나 )에 부모 요소인 dt태그 다음에 오는 
	  //dd태그를 show메소드로 노출하여 보여주자
	  $(this).parent().next().show();
	  //마우스를 올리거나 클릭한 a태그에 하위 버튼 img태그를 선택해 활성화된 이미지로 바꿉니다.
	  $("img",this).attr("src",$("img",this).attr("src").replace("out.gif","over.gif"));//하위 이미지들중에
	  
	  //탭버튼의 a태그들 공지사항.질문과 답변, 저자문의 중에 마우스를 올리거나 포커스또는 클릭했을경우
	   //클릭할때마 클릭한 요소 a를 선택해서 ontab변수에 다시 할당 합니다. 
	  onTab=$(this);
	  //a링크 기능을 차단
	  return false;
  });
...
*/
 ...
 /* $("#best_bg ul")로 베스트 북 목록 태그인 ul태그를 선택하여
  bx슬라이더 메소드 적용하고 슬라이드 옵션을 지정하면된다.*/
  var mySlider =$("#best_bg ul").bxSlider({
	  
	  										//속성명 : 설정값
	  										mode: "horizontal",//수형 방향으로 이동시키기
	  										speed: 500,//이동속도 0.5초
	  										moveSlides:1,//이동시킬 슬라이드수 설정 
	  										slideWidth: 125,//슬라이드 폭
	  										minSlides:1,//최소 노출되는 슬라이드수
	  										maxSlides:3,//최대 노출되는 슬라이드수
	  										slideMargin:30,//슬라이드 간의 간격 
	  										auto:true,//자동슬라이드 여부(true: 자동, false:수동)
	  										autoHover:true,//마우스 오버시 자동 정지 여부 
	  										pager:false, //페이징 표시를 제어 false:숨김 ,true :노출
	  										controls:false//이전 prev, 다음 next 노출 여부 (true 노출 ,false: 숨김 )
  ...
  $(".prev_btn").on("click",function(){
	  //gotoprevslide()메소드를 이용하여 슬라이드를 한단계 이전으로 이동되게하기
	  mySlider.goToPrevSlide();
	  return false;
  });
  
  $(".next_btn").on("click",function(){
	  //gotoNextslide()메소드를 이용하여 슬라이드를 한단계 이전으로 이동되게하기
  ...
  */
   ...
   /*팝업 연동*/
  //설명 :저장된 pop에 쿠키값을 검사하여 만일 쿠기가 저장되어 있지 않으면 숨겨져있던 팝업창이 보이도록한다.
  //그리고 사이트 방문객이 (하루동안 이창 열지 않기 )눌렀을떄 쿠키를 생성해서 웹브라우저에 하루동안 보관 되도록함 
  if($.cookie("pop")!="no")
	  //만약 팝이라는 쿠키명에 해당하는 쿠키값 no가 웹브라우저에 저장되어있지 않다면 
	  // 숨겨져 있던 팝업창 영역을 show함수를 이용해 노출시키자. 
	  $("#pop_wrap").show();

  //팝업창 영역에 마우스 포인터를 올리면 커서 모양으로 바뀌게 하기 
  //그리고 draggable()메소드를 이용하여 마우스로 드래그 했을때 팝업창이 부드럭베 이동되도록 해보자
